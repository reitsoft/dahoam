<script lang="ts">
	...
	// Echte Daten kommen jetzt als Props (z.B. aus +page.server.ts via load())
	let {
		verbrauchProTag,
		verbrauchVormonat
	}: {
	...
	const dataPoints = verbrauchProTag.map((d) => d.value);
	const labels = verbrauchProTag.map((d) => String(d.tag));

	const VERBRAUCH_MONAT = Math.round(
		dataPoints.reduce((sum: number, v) => sum + (v ?? 0), 0) * 10
	) / 10;
	const VERBRAUCH_LETZTER_MONAT = Math.round(verbrauchVormonat * 10) / 10;
...
<div class="flex h-full flex-col overflow-hidden rounded-2xl bg-navy-900 p-3">
	<div class="flex shrink-0 items-center justify-between px-1 pb-1">
		<div class="flex items-center gap-3">
			<div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-cream-100/10 text-cream-100/70">
		...
		<div class="flex items-center gap-3">
			<div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-cream-100/5 text-cream-100/40">
